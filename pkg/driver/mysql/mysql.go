...
import (
	...
	"github.com/ArkhamKnight25/dbmigrate/pkg/dbmate"
...
func (drv *Driver) CreateMigrationsTable(db *sql.DB) error {
	_, err := db.Exec(fmt.Sprintf(
		"create table if not exists %s (version varchar(128) primary key, checksum varchar(64) not null default '', applied_at timestamp not null default current_timestamp, dirty boolean not null default false)",
...
func (drv *Driver) SelectMigrations(db *sql.DB, limit int) (map[string]dbmate.MigrationRecord, error) {
	query := fmt.Sprintf("select version, checksum, dirty from %s order by version desc", drv.quotedMigrationsTableName())
	if limit >= 0 {
	...
	migrations := map[string]dbmate.MigrationRecord{}
	for rows.Next() {
		var record dbmate.MigrationRecord
		if err := rows.Scan(&record.Version, &record.Checksum, &record.Dirty); err != nil {
			return nil, err
		}
...
func (drv *Driver) InsertMigration(db dbutil.Transaction, version string, checksum string) error {
	_, err := db.Exec(
		fmt.Sprintf("insert into %s (version, checksum, dirty) values (?, ?, false)", drv.quotedMigrationsTableName()),
		version, checksum)
