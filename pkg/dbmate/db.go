...
var (
	...
	ErrDirtyMigration        = errors.New("dirty migration state detected")
	...
	for _, migration := range pendingMigrations {
		...
		for _, migrationSection := range parsed {
			...
			execMigration := func(tx dbutil.Transaction) error {
				...
				// record migration
				return drv.InsertMigration(tx, migration.Version, checksum)
			}
...
func (db *DB) readMigrationsDir(dir string) ([]fs.DirEntry, error) {
	...
	// This supports both relative and absolute migration directory paths.
	if db.FS == nil {
...
func (db *DB) FindMigrations() ([]Migration, error) {
	...
	appliedMigrations := map[string]MigrationRecord{}
	...
	for _, dir := range db.MigrationsDir {
		...
		for _, file := range files {
			...
			if record, ok := appliedMigrations[migration.Version]; ok {
				if record.Dirty {
					return nil, fmt.Errorf("%w: %s", ErrDirtyMigration, migration.Version)
				}
				checksum, err := migration.Checksum()
				if err != nil {
					return nil, err
				...
				if record.Checksum != "" && checksum != record.Checksum {
					return nil, fmt.Errorf("%w: %s", ErrChecksumMismatch, migration.FileName)
				}
